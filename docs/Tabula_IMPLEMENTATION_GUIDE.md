# GAM 구현 가이드 (MK GPT 레이아웃 공통)

> 대상: 매일경제 **View GPT / HOME GPT / Section GPT / List GPT** 템플릿을 구현·운영하시는 프론트엔드·광고 개발자님  
> 작성자 관점: **광고기획자 → 광고개발자에게 설명**한다는 전제
...
이 가이드는 앞으로 **MK 디지털 전 채널의 GAM 인프라를 표준화**하기 위한 첫 버전입니다.  
추후 다음과 같은 방향으로 계속 업데이트해 나갈 예정입니다.

- Prebid/오픈빗 통합 시의 **헤더비딩 연동 규칙** 추가
- 1st-party 데이터 및 PPID 확장 전략 정리
- SPA/Infinite Scroll 환경에서의 광고 재요청/리프레시 정책

실제 구현 중 막히는 부분, 혹은 기획 단계에서 추가로 정의가 필요한 부분이 있으시면  
**“어떤 위치에 어떤 비즈니스 룰을 적용하고 싶은지”**를 편하게 말씀해 주세요.  
그에 맞춰 슬롯 구조·타게팅·로딩 전략까지 함께 설계해 보겠습니다.
