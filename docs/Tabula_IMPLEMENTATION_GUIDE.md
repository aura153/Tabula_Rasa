...
#### 4.5.1 가로형 배너용 – HOME / Section / List / View 공통 개념

```js
const sizeMapping_horizontal = googletag.sizeMapping()
  .addSize([1024, 0], [
    // 데스크탑 (와이드·빌보드·리더보드 중심)
    [1200, 300], [1200, 100],
    [970, 250], [980, 120], [970, 90], [930, 180],
    [728, 90],
    [2, 1], [1, 1], 'fluid'
  ])
  .addSize([768, 0], [
    // 태블릿 (750 폭 + 728x90 + 중형 직사각형)
    [750, 300], [750, 200],
    [728, 90],
...
- **용도**
  ...
  - Section GPT / List GPT:  
    - 상단 `TC_billboard_leaderboard` 및 필요 시 중단 가로형 배너에 적용
  - View GPT:  
    - 상단 `TC_billboard_X`, 하단 `BC_billboard_rectangle`, byline 하단 일부 영역에 적용
- **브레이크포인트 개념**
  - `1024px 이상 (데스크탑)`  
...
const sizeMapping_square = googletag.sizeMapping()
  ...
  .addSize([0, 0], [
    // 모바일 – 336×280 / 320×480 / 300×25x 시리즈까지 지원
    [336, 280], [320, 480],
    [320, 100], [320, 50],
    [300, 250], [250, 250], [200, 200],
    [2, 1], [1, 1], 'fluid'
...
- **용도**
  - HOME GPT  
    - `MR_halfpage_rectangle`에 사용 (코드 상 이름은 `sizeMapping_right_square`)
  - Section GPT / List GPT  
    - 우측 `MR_halfpage_rectangle`(사이드 레일 광고)에 동일 정책 적용
  - View GPT  
    - 우측 레일 `MR_side_rail_X`에 `sizeMapping_siderail_square`로 적용  
      (실제 코드는 위 정책과 거의 동일한 구성)
- **디자인/레이아웃 전제**
  - HOME / Section / List / View 공통 CSS에서  
    사이드바/우측 레일 폭은 **300~336px**로 고정 또는 최소 보장:
    - HOME: `.sidebar { flex: 0 0 300px; min-width: 300px; }`
    - View: `.related-area { flex: 0 1 336px; min-width: 336px; }`
  - 이 폭 정책을 전제로 300×600, 300×250, 336×280 등이 안정적으로 노출되도록 설계

---

#### 4.5.3 In-Article 혼합형 – View GPT 전용 (MC in-article)

```js
const sizeMapping_article_horizontal = googletag.sizeMapping()
  .addSize([1024, 0], [
    // 데스크탑 – in-article 중형 직사각형 + 변형 사이즈
    [480, 320], [336, 280],
    [320, 100], [320, 50],
    [300, 250],
    // 300x25x 시리즈: 캠페인/라인아이템 세분화용
    [300, 251], [300, 252], [300, 253], [300, 254],
    [300, 255], [300, 256], [300, 257], [300, 258],
    [250, 250],
    [2, 1], 'fluid'
  ])
  .addSize([768, 0], [
    // 태블릿 – 동일 계열 + 200×200 추가
    [480, 320], [336, 280],
    [320, 100], [320, 50],
    [300, 250],
    [300, 251], [300, 252], [300, 253], [300, 254],
...
- **특징**
  - **In-article 전용 혼합형 매핑**
    - 300×250 / 336×280 / 480×320 같은 전형적인 중형 배너 +  
